...
use crate::core::{Path, PathBuilder, Point, Rect, Scalar, FillType, Verb};
    ...
    fn next_edge(
        ...
        for (i, verb) in self.path.verbs().iter().enumerate() {
            ...
            let pts: [Point; 4] = std::array::from_fn(|j| {
                self.path.points().get(i + j).copied().unwrap_or(Point::default())
            });
                ...
                for j in 0..(verb.point_count() - 1) {
                    if let (Some(p0), Some(p1)) = (
                            ...
                            || (p0.y >= contour.min_xy.y && p1.y < contour.min_xy.y)
                        {
                            ...
                            let intersect_x = p0.x + (contour.min_xy.y - p0.y)
                                * (p1.x - p0.x)
                                / (p1.y - p0.y);
